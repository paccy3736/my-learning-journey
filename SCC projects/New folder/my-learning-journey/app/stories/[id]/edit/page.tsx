...
export default function EditStoryPage() {
  ...
  useEffect(() => {
    getStory(id).then((story) => {
      if (!story) {
        ...
        setForm({ authorName: story.authorName, content: story.content });
      }
      setLoading(false);
    });
  }, [id]);
  ...
  if (loading) return <div className="mx-auto max-w-xl px-4 py-10 text-slate-500">Loading...</div>;
  if (notFound) return <div className="mx-auto max-w-xl px-4 py-10"><p className="text-slate-500">Story not found.</p></div>;

  return (
    <div className="mx-auto max-w-xl px-4 py-10">
      <div className="mb-6">
        <button onClick={() => router.back()} className="mb-4 text-sm text-slate-500 hover:text-blue-600 transition-colors">
          ← Back
        </button>
